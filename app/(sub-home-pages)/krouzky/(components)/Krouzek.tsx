...
export default function Krouzek(props:krouzek){
    return(
    <Link href={"/krouzky/"+props.id}>

        <div className="bg-white py-6 px-4 rounded-2xl drop-shadow-md flex flex-col items-center max-w-sm ">
            <div className="w-full">
                <h3 className="text-xl underline font-medium md:text-base md:font-normal text-start w-full">{props.title}</h3>
            </div>
            ...
            <button className=" w-full md:w-11/12 bg-black text-white py-2 rounded-md drop-shadow-md md:hidden">Zobrazit kroužek</button>
            <div className="hidden md:inline text-sm text-start self-start underline font-light tracking-tight"><a>Zobrazit kroužek {'>'}</a></div>
        </div>
    </Link>
    )
}